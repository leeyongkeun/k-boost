...
- imageKeyword는 반드시 영어로, 해당 카드 내용에 맞는 한국 관광 관련 이미지 검색어를 작성하세요.`;
    ...
    // 배경 이미지를 서버에서 fetch → base64 변환 (CORS 문제 해결 + html2canvas 호환)
    const imagePromises = cards.map(async (card) => {
      try {
        const keyword = card.imageKeyword || "korea tourism";
        // Pexels-style free image via loremflickr (키워드 지원, API키 불필요)
        const url = `https://loremflickr.com/1080/1080/${encodeURIComponent(keyword)}`;
