...
function buildAreaInfo(areaContext?: GeminiSearchInput["areaContext"]): string {
  if (!areaContext) return "";
  return `\n참고 상권 데이터: ${areaContext.city} ${areaContext.districtName}, 관광등급 ${areaContext.touristRank}, 외국인 비율 ${Math.round(areaContext.foreignVisitorRatio * 100)}%, 일 유동인구 ${areaContext.dailyFootTraffic.toLocaleString()}명`;
}
...
export async function searchAndAnalyzeStore(
  ...
  try {
    ...
    const areaInfo = buildAreaInfo(input.areaContext);
...
- 뻔한 일반론 금지. 이 매장의 실제 데이터에 기반한 구체적 조언만.`;
    ...
    const parsed = parseAiJson(text);
    if (!parsed || !validateParsed(parsed)) return null;

    // Gemini 검색 모드는 platforms를 응답에서 가져옴
    const rawPlatforms = parsed.platforms as { name: string; registered: boolean; score?: number; reviewCount?: number; hasPhotos?: boolean; hasEnglish?: boolean }[];
    const platforms: PlatformInfo[] = (rawPlatforms || []).map((p) => ({
      name: p.name,
    ...
    return processGeminiResponse(
      text,
      input.storeName,
      (parsed.business_type as string) || "매장",
      platforms,
    );
  } catch (error) {
...
- 뻔한 일반론 금지. 이 매장에 맞는 구체적 조언만.`;
    ...
    const inputPlatforms: PlatformInfo[] = input.platforms.map((p) => ({
      name: p.name,
      registered: p.registered,
      score: p.score,
      reviewCount: p.reviewCount || 0,
